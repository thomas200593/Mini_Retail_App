...
class VMConfGenTimezone @Inject constructor(
    ...
    data class UiState(
        val configTimezone: ResourceState<ConfigTimezones> = Idle,
    ...
    private fun onOpenEvent() = viewModelScope.launch(ioDispatcher) {
        ...
        ucGetConfTimezone.invoke().flowOn(ioDispatcher)
            .catch { e ->
                _uiState.update { it.copy(configTimezone = Error(e)) }
                updateDialogState(
            ...
            .collect{ data ->
                _uiState.update { it.copy(configTimezone = data, dialogState = DialogState()) }
            }
    }
    private fun onBtnNavBackClicked() {
        _uiState.update { it.copy(dialogState = DialogState()) }
    }
    private fun onSaveSelectedTimezone(timezone: Timezone) = viewModelScope.launch(ioDispatcher){
        repoConfGenTimezone.setTimezone(timezone)
    }
